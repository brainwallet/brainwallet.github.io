...
/*
    ...
    Easy way to get all that: http://blockchain.info/unspent?address=<address>
    Hard way: in development (e.g. scraping blockexplorer.com to death).
...
var TX = new function () {
    ...
    this.addOutput = function(addr, fval) {
        outputs.push([addr, fval]);
    }

    this.getBalance = function(text) {
        var inputs = jQuery.parseJSON(text).unspent_outputs;
        var sum = new BigInteger('0', 10);
        for (var i in inputs) {
            var o = inputs[i];
    ...
    this.construct = function(inputs) {
        var sendTx = new Bitcoin.Transaction();
        var sum = new BigInteger('0', 10);
        var scripts = [];
        for (var i in inputs) {
            var o = inputs[i];
            var hash = Crypto.util.hexToBytes(o.tx_hash);
            var script = Crypto.util.hexToBytes(o.script);
            var recv = new BigInteger('' + o.value, 10);
            sum = sum.add(recv);
            ...
            scripts.push(new Bitcoin.Script(script));
            sendTx.addInput(txin);
        }

        for (var i = 0; i < outputs.length; i++) {
            var addr = outputs[i][0];
            var fval = outputs[i][1];
            ...
            if (value instanceof BigInteger) {
                ...
                //value = value.reverse();
                //console.log(Crypto.util.bytesToHex(value));
            }
            */
...
var tx_addr = '15ubicBBWFnvoZLT7GiU2qxjRaKJPdkDMG';
var tx_dest = '1HB5XMLmzFVj8ALj6mfBsbifRoD4miY36v';
...
function tx_test() {
    ...
    var r = jQuery.parseJSON(tx_unspent);
    var sendTx = TX.construct(r.unspent_outputs);
