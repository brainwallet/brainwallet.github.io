...
var BIP32 = function(bytes) {
    ...
    if( typeof bytes === "string" ) {
        ...
        if( decoded.length != 82 ) throw new Exception("incorrect data size: data must be 82 bytes"); 
    ...
    if( bytes !== undefined )
        throw new Exception("Expected an array object");

    // `bytes === undefined' is used when deriving child bip32s, so no error here
}

BIP32.prototype.init_from_bytes = function(bytes) {
    // Both pub and private extended keys are 78 bytes
    if( bytes.length != 78 ) throw new Exception("incorrect data size: data must be 78 bytes");
