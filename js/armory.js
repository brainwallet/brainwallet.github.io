 (function($){
	...
	var FIXED_COSTS=500000;
	var VARIABLE_COSTS=50000;
	var CONFORMWITHPROTOCOL = false;
	var POSTFEE = false;
	
	function toSatoshis(chunkArr){
		var mySatoshiArray = new Array();
		
		if(POSTFEE)
			mySatoshiArray.push(0);
		
		for (var i=0;i<chunkArr.length;i+=4)
		{
			var sumMe = parseInt(chunkArr[i]+chunkArr[i+1]+chunkArr[i+2]+chunkArr[i+3],16);
			mySatoshiArray.push(sprintf('0.'+'%1$08d', sumMe));
		}
		
		if(POSTFEE)
			mySatoshiArray[0] = ((0.0+FIXED_COSTS+mySatoshiArray.length*VARIABLE_COSTS)/10000000).toString();
		
		return mySatoshiArray;
	}
	
	function satoshiToData(satoshiArr){
		
		var cleanData = new Array();
		for (var i=0;i<satoshiArr.length;i++)
		{	
				cleanData.push(sprintf('%1$04x', parseInt(sprintf('%1$05d', parseFloat(satoshiArr[i].slice(3))))));
		}
		return cleanData;
	}
	...
	function extractTransactionValues(transaction){
		var cleanData = new Array();
		for (var i=0;i<transaction.length;i++)
		{
					cleanData.push(transaction[i].value);
		}
	...
	function setTxMsg(){
		var inputData;
		if($('#msgAsHex').attr('checked') == 'checked'){
			inputData = (CONFORMWITHPROTOCOL?"02":"")+($('#txMessage').val());
		}
			else
		{
			inputData = (CONFORMWITHPROTOCOL?"02":"")+Crypto.util.bytesToHex(strToBytes(($('#txMessage').val())));
		}
		var myEncoded = toSatoshis(inputData);
		
		var list = $(document).find('.txCC');
		var clone = list.last().clone();
		list.first().find('#txValue').val(myEncoded[0]);
		
		for(var i=1;i<myEncoded.length;i++){
			list = $(document).find('.txCC');
			clone = list.last().clone();
	...
	function readTxMsg(){
		var jsonObject = $('#msgReadTransaction').val();
		var jsonObjectOut;
		
		if(jQuery.parseJSON(jsonObject).out == null)
		{
		var re = new RegExp('("value" : )(.*)(,)', "g");
		jsonObject = jQuery.parseJSON(jsonObject.replace(re, '$1"$2"$3'));
		jsonObjectOut = jsonObject.vout;
		}
		else
		{
		jsonObjectOut = jQuery.parseJSON(jsonObject).out;
		}
		
		var data = satoshiToData(extractTransactionValues(jsonObjectOut)).join("");
		if($('#msgReadtoStr').attr('checked') == 'checked'){
			var pureText = bytesToString(Crypto.util.hexToBytes(data));
			$('#msgReadTextArea').val(pureText);
			return;
		}
		$('#msgReadTextArea').val(data);
	}
    ...
    $(document).ready( function() {
        ...
        // generator
		$('#msgToggle').click(setTxMsg);
		$('#msgRead').click(readTxMsg);
